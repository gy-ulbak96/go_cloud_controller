...
const (
	// SuccessSynced is used as part of the Event 'reason' when a Server is synced
	SuccessSynced = "Synced"
	// ErrResourceExists is used as part of the Event 'reason' when a Server fails
	...
	// MessageResourceSynced is the message used for an Event fired when a Server
...
func NewController(
	...
	// Set up an event handler for when Server resources change
...
// converge the two. It then updates the Status block of the Server resource
...
func (c *Controller) syncHandler(key string) error {
	...
	// Get the Server resource with this namespace/name
	server, err := c.serversLister.Servers(namespace).Get(name)
	if err != nil {
		// The Server resource may no longer exist, in which case we stop
		// processing.
...
//------------------------------------------------------------------
  ...
  fmt.Println(realserver)
	...
	// if errors.IsNotFound(err) {
	// 	deployment, err = c.kubeclientset.AppsV1().Deployments(server.Namespace).Create(context.TODO(), newDeployment(server), metav1.CreateOptions{})
	// }
	...
	// if !metav1.IsControlledBy(deployment, server) {
	...
	// if server.Spec.Replicas != nil && *server.Spec.Replicas != *deployment.Spec.Replicas {
	// 	klog.V(4).Infof("Server %s replicas: %d, deployment replicas: %d", name, *server.Spec.Replicas, *deployment.Spec.Replicas)
	// 	deployment, err = c.kubeclientset.AppsV1().Deployments(server.Namespace).Update(context.TODO(), newDeployment(server), metav1.UpdateOptions{})
	// }
...
// func newDeployment(server *samplev1alpha1.Server) *appsv1.Deployment {
...
// 			Name:      server.Spec.DeploymentName,
// 			Namespace: server.Namespace,
// 			OwnerReferences: []metav1.OwnerReference{
// 				*metav1.NewControllerRef(server, samplev1alpha1.SchemeGroupVersion.WithKind("Server")),
// 			},
// 		},
